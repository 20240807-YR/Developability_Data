# core/core_11/run/run_core11_데모3.py
...
def load_core6_candidates(core6_csv: Path) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    agg["fallback_score_n"] = norm01(agg["fallback_score"])
...
def main():
    ...
    if not SCENARIO_KEY or not RUN_ID:
        raise RuntimeError("CORE11_SCENARIO and CORE11_RUN_ID must be set")
    ...
    for step in range(int(cfg.t_steps)):
        ...
        if policy_fb:
            sel, alts, score, src = pick_core6_fallback(
                core6_ok, core6_all, used_core6, TOP_K, ALLOW_USE_CRITICAL
            )
